...
Implementation of the **PositionalList interface** relies on a **doubly linked list.**

{% code title="Implementation of a positional list stored as a doubly linked list." %}
```java
...
/** Returns the given node as a Position (or null, if it is a sentinel). */ 
  ...
  public E set(Position<E> p, E e) throws IllegalArgumentException {
    Node<E> node = validate(p); E answer = node.getElement(); node.setElement(e);
    return answer;
  }
...
The positional list ADT is ideally suited for implementation with a doubly linked list.

{% hint style="info" %}
All operations run in **worst-case constant time.**
{% endhint %}

![Performance of a positional list with n elements realized by a doubly linked list. The space usage is O\(n\).](../.gitbook/assets/jie-ping-20210706-xia-wu-3.42.38.png)
